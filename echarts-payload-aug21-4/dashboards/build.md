...
```text
Tool 1  author pulls.json → persist → run every entry → verify CSVs
   ↓
...
`scripts/pulls.json` declares what the dashboard retrieves. Write it with `apply_pulls_document`, which validates every entry against the `get_data` `Details` union before anything lands, then run each entry and verify the CSV it produced.
...
written = apply_pulls_document(FOLDER, {
    "schema_version": 1,
    "pulls": [{
        "name": "rates",
        "details": {
...
- Declare only verified identifiers. An exact expression/code/client call supplied by the user or test prompt is authoritative for that task; preserve it byte-for-byte. If no identifier is supplied or verified, stop at the external-data boundary and ask for it—never infer a vendor code from a label or naming pattern.
- Per-series `label` is the preferred place to establish plain-English persisted columns.
- `written["declared_stems"]` is the exact set of dataset keys the template must use, and the set the refresh audit will hold the dashboard to.
- Use an `@today`-family token for any window that should move. A frozen literal `end` makes every future refresh report success while the data stops advancing.
...
- One entry per source-and-failure unit: an entry is the cycle's only failure boundary, so two sources with independent availability belong in two entries.
...
- Only CSV files become build datasets. Metadata sidecars and `df.attrs` are not loaded and never populate `field_provenance`; author lineage in each template dataset entry.

An exact user-supplied deterministic fixture is verified source data with no vendor identifier and nothing to retrieve, so it is a `save_artifact` write in `build.py`, not a pull. Preserve supplied values and nulls without normalization:
...
def load_fixture(datasets):
    frame = pd.DataFrame(EXACT_USER_SUPPLIED_ROWS)
    save_artifact(frame, name="fixture", output_path=f"{SESSION_PATH}/data",
                  s3_manager=s3_manager)
    datasets["fixture"] = frame
    return datasets

TRANSFORMS = [load_fixture]
```

`dashboards/pipelines.md` owns the full stem table, including the `save_artifact` return-shape condition and the multi-table exception.

### Tool-only build

A pure calculator with no source data is a legal first build. Persist both files with these exact declarations:

```python
# scripts/pulls.json
{"schema_version": 1, "pulls": []}
...
widgets. Call `build_dashboard` directly in Tool 2; there is no pull to run
and no placeholder CSV to create. The empty document is still required —
"declares no pulls" and "the file is missing" are different states. Persisted
