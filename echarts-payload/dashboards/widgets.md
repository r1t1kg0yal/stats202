...
### Widget shape & width contract (read before authoring a row)

These rules are ENGINE-ENFORCED by the manifest validator and are intentional legibility guards, not bugs. Author to them; a rejection here is a contract reminder, not a defect to file.

- **Each row is a LIST of widget dicts** (`layout.tabs[].rows[]` -> `[{widget...}, {widget...}]`), never a single widget dict. An inspection script must iterate the inner list; calling `.get()` on a row raises `AttributeError`.
- **`chart` widgets must be exactly 2-up (`w = cols/2`) or 3-up (`w = cols/3`).** A chart may NEVER span the full row or any other width -- full-width charts flatten slopes into zero-slope ribbons. Omitting `w` on a chart defaults to 2-up, which is legal. To honor a "drop the companion, widen the chart" request, do NOT widen the chart; instead pair it with another full-height widget so the row stays 2-up/3-up. **Valid full-width-capable companions** (any width, including a full row on their own): `kpi`, `table`, `pivot`, `stat_grid`, `markdown`, `image`, `divider`. Use one of these as the partner, or move the chart to its own 2-up row with a sibling chart.
- **`kpi` and `stat_grid` numeric sources must resolve to numeric values** (the source column is coerced via `to_numeric`; an all-text column is rejected with `kpi_source_no_numeric_values`). For a single-row CATEGORICAL status (e.g. `"affirmative"` / `"in-progress"`), there is no numeric tile -- use a `markdown` widget with a semantic `kind` (`insight` / `watch` / `risk` / `fact`, §8) which renders a coloured-stripe status card, not a `kpi`/`stat_grid` tile.
- **`dataset` field name is chart-only.** `table` and `pivot` widgets accept `ref` or `dataset_ref` (NOT `dataset`); a `table` with a `dataset` key fails with `table_widget_requires 'ref' or 'dataset_ref'`. This asymmetry is deliberate: chart specs bind a `dataset`, tabular widgets bind a `dataset_ref`.
...
manifest["datasets"]["rates"] = {
    "source": df_rates,
    "field_provenance": {
        "UST10Y": {"system": "tsdb", "symbol": "fi_usgov_gt10",
                    "tsdb_symbol": "fi_usgov_gt10", "display_name": "US 10Y Treasury Rate",
                    "units": "percent", "source_label": "TSDB (via PlotTool)"},
        "JCXFE":  {"system": "haver", "symbol": "<CODE>@<DATABASE>",
                    "haver_code": "<CODE>@<DATABASE>", "units": "percent",
...
| `symbol` | Universal primary identifier - pass the exact upstream string (abstract shape: `<CODE>@<DATABASE>` for Haver, `<COORDINATE>` for GS Market Data, `<FRED_ID>` for FRED, `<bloomberg_ticker>` for Bloomberg, `<factset_id>` for FactSet; never invent codes from pattern) |
