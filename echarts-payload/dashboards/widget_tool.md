...
**`compute_js` lives ONLY in `manifest_template.json`. `build.py` never authors it.** The new `build.py` shape (hub §B Tool 2) is `TRANSFORMS = [<derive_fn>, ...]` — transforms mutate the datasets dict, full stop. They do NOT concatenate, f-string, or `.format()` anything into a tool's `compute.source`. The compute body is written ONCE at first dashboard creation (when the initial manifest dict is composed in Tool 2 before `manifest_template()` strips data) and edited thereafter via surgical READ → MERGE → WRITE on `manifest_template.json` (hub §C). Inlining Python values into a JS body produces a ReferenceError at runtime: `None` / `True` / `False` / `nan` / `inf` / `Timestamp(...)` / `Decimal(...)` are not defined identifiers in JavaScript, and the runtime fails with `compute error: <token> is not defined` while the right panel of the tool tile renders blank. The engine validator hard-blocks the leak class at validate time (`tool_compute_python_literal_in_js`); the new build.py shape prevents it from being written in the first place — transforms cannot reach the manifest's `tool_def.compute_js` because they receive only the `datasets` dict, never the manifest.
