...
| Prior `manifest_template.json` was quarantined to `archive/<UTC>/` (§2.5.2 of dashboards.md) | Pure-template edit (no script change) made via §3 WRITE that the user wants to undo | Read the archived file, validate it, restore via §3 WRITE step. Scripts are unaffected; no version bump |
