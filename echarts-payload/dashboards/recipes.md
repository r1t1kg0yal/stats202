...
- Rename opaque API codes to plain English (or pass `labels=` at the pull layer)
...
# 1. AUDIT (dashboards_hub.md § 2.5.3 -- load existing manifest, run audit; raises if non-compliant)
...
# 6. VERIFY via in-session quick recompile (dashboards_hub.md § 6.6 / template_crud.md § 9)
#    exec(build.py from S3) confirms the change compiles against current data.
```

`scripts/build.py` is touched only when the dataset shape it builds against has changed (a new dataset key was added, a key was removed, or a column was renamed). Pure widget / filter / layout edits leave `build.py` alone. When `build.py` (or `pull_data.py`) does need to change, the rule is `dashboards_hub.md` § 2.5.5 -- surgical READ → MUTATE → WRITE, never wholesale re-emission. See § 6 below for the worked recipe.
...
| Prior `manifest_template.json` was quarantined to `archive/<UTC>/` (§2.5.2 of dashboards_hub.md) | Pure-template edit (no script change) made via §3 WRITE that the user wants to undo | Read the archived file, validate it, restore via §3 WRITE step. Scripts are unaffected; no version bump |
| User can describe the prior layout in chat | The user-described prior state has neither a script-version snapshot nor a `history/` / `archive/` source | Re-build the prior `manifest_template.json` from scratch following the description; surface a diff vs current state before writing. If the description implies a script change, route through Path 1 by treating the rebuild as a fresh Tools 1+2 cycle |
...
# 1. AUDIT current state (per dashboards_hub.md §2.5.3)
m = json.loads(s3_manager.get(f"{DASHBOARD_PATH}/manifest.json").decode("utf-8"))
_audit_dashboard_layout(DASHBOARD_PATH, m)
...
# 6. RUN Tools 1+2+3+4 (dashboards_hub.md §6.1) to validate against today's
...
When the dataset shape changes (new column, new pull source, new derived dataset, column rename, pull-fn swap), scripts are mutated SURGICALLY -- never re-emitted from a fresh string (`dashboards_hub.md` §2.5.5). The script-rebuild path is reserved for first-time creation (Tools 1+2+3+4) and total demolition.

Six steps:

1. AUDIT (`dashboards_hub.md` §2.5.3) -- folder must be compliant before any mutation
...
new_src = src.replace(
    "codes=[\n        '<CODE>@<DATABASE>', # Effective Fed Funds Rate\n"
    "        '<CODE_2>@<DATABASE>', # SOFR\n   ],",
    "codes=[\n        '<CODE>@<DATABASE>', # Effective Fed Funds Rate\n"
    "        '<CODE_2>@<DATABASE>', # SOFR\n"
    "        '<CODE_3>@<DATABASE>', # 3-month T-bill\n   ],",
)
...
assert '<CODE_3>@<DATABASE>' in df.columns or 'tbill_3m_pct' in df.columns
```
...
`build.py` is not a thin loader. After loading raw CSVs from `pull_data.py`'s output, `build.py` is the right surface for **derivation** -- anything that can be computed from existing pulls without another network call. Hand-typed `value: <num>` in the manifest is forbidden (`dashboards_hub.md` Rule 1; codes `kpi_static_value_forbidden` / `stat_grid_static_value_forbidden`). Derivation in `build.py` replaces that footgun with refresh-safe computation.
