...
`make_chart()`, `make_table()`, every annotation class (`ChartSpec`, `VLine`, `HLine`, `Band`, `Callout`, `Segment`, `Arrow`, `PointLabel`, `PointHighlight`, `LastValueLabel`, `Trendline`, `PlotText`), composite + `profile_df` helpers are AUTO-INJECTED into the sandbox namespace -- call them bare. Do NOT `import` any of them (no `from chart_context import ...`, no `from prism_charts import ...`): there is no such module and the import RAISES `ModuleNotFoundError` before any chart renders. Raw matplotlib blocked. `s3_manager` / `session_path` / `user_id` auto-inject at call time -- never pass them.

## Hard limits (anticipate these BEFORE authoring -- the engine raises, it never truncates)

The engine enforces these caps up-front and **raises** rather than silently clipping. Shortening strings in the DataFrame / kwargs is always the fix.

| Limit | Cap | Trips | Fix |
|---|---|---|---|
| Lines per `multi_line` / `area` panel | **4** | 5+ series on one canvas; LVL labels collide | composite (§10), facet (grids spoke), or keep top 4 |
...
| Composite small-pack ceiling vs facet floor | small packs up to **4** panels; 5+ entities same shape | 7 G7 lines on one panel, 8-panel ranking | `make_*pack_*` ≤4 cells or facet (grids spoke) |
...
## 2a. Batch building -- collect every failure in one run (MANDATORY for 2+ charts)

A bare sequence of `make_chart` calls stops at the **first** `ValidationError` -- every later chart is never attempted and its failure is invisible. When building 2+ charts in one script, define every chart as a `(name, thunk)` pair and drive them through `build_charts()` so **one run surfaces every failure**.

```python
def build_charts(chart_specs):
    """Build every chart spec; aggregate failures; raise once if any failed."""
    survivors = []
    failures = []
    for name, thunk in chart_specs:
        try:
            r = thunk()
            survivors.append((name, r))
...
**Authoring pattern.** Declare specs first (names label the aggregate error; use short snake_case). Each `thunk` is a zero-arg callable that invokes exactly one `make_chart` / `make_*pack_*` call with all kwargs wired:
...
chart_specs = [
    ...
    ("eu_cpi", lambda: make_chart(
        df=eu_cpi, chart_type='multi_line',
        mapping={'x': 'date', 'y': 'value', 'color': 'series'},
        title='EU HICP YoY', save_as='charts/eu_cpi.png',
    )),
...
built = build_charts(chart_specs)
```

**QC the survivors.** Only charts that returned from `build_charts` exist on disk. Gate them in one pass:
...
If `build_charts` raises, fix **every** named failure before re-running the batch -- do not cherry-pick one chart and re-run hoping the others were fine.

**Anti-pattern:** `for spec in specs: make_chart(...)` with no harness -- first failure aborts silently for the rest. **Anti-pattern:** wrapping a single eager chart in `try/except` to "be safe" -- let single-chart failures bubble per §1.
...
- **Building 2+ charts in one script? Use the batch-build harness (§2a)** instead of a bare sequence -- one run surfaces every failure, not just the first.
...
> **Harness vs. composite-raise interaction.** The §2a batch harness wraps WHOLE chart calls -- including a `make_*pack_*` composite call as a single thunk. When that thunk raises, the traceback is one composite-level `ValidationError` naming how many sub-charts failed. To find which *panel* failed, read `chart_errors` on the surfaced traceback / error message (the engine folds per-cell `error_message` values in). Build every `ChartSpec` from validated, non-empty data before composing; the harness does not partial-render composites.
...
With the batch harness (§2a), the retry budget is per *batch*, not per chart -- one fix pass addresses every named failure, then rebuild the whole batch.
