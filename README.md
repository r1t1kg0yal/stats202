...
│                                       INSTALLED PARITY OPEN: live │
│                                       prism-core checkout differs │
│                                       from parent-recorded SHA;   │
│                                       no zero-delta claim.        │
...
| echarts | mature + time/live-refresh uplift; split-package local harness landed; live architecture re-verified 2026-07-11; installed-byte parity remains open because the checked-out submodule SHA differs from the parent-recorded SHA | `projects/echarts/` | `projects/echarts/echarts-payload/` (8 Python files + 11 context files: registered router, on-demand kernel, 9 spokes; 8-file `test_prompts/` remains staging-only) | 7 modules in `prism-core/dashboards/` + parent-tree `jobs/hourly/refresh_dashboards.py` + `prism-core/context/modules/static/tools/{dashboards,dashboards_hub}.md` + `tools/dashboards/*.md`; local runtime mirrors the split checkout through `core/`, `dashboards/`, `jobs/`, `prism_mcp/`, `prism_meta/`, and `web/` | `.cursor/rules/viz-platforms.mdc` | `codebase-tree.md` §0+§5, `code-sandbox.md` §2.4+§8, `mcp-tools.md` §5, `dashboard-refresh.md`, `dashboards-portal.md` | `staging/prompts/open/2026-07-11_dashboard_architecture_validation.md` (partially resolved; CDN + commit-baseline closure open) |
