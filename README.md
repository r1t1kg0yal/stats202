...
| echarts | per ownership/test surface | 8 (`charts`, `diagnose`, `filters`, `pipelines`, `recipes`, `template_crud`, `widget_tool`, `widgets`) — 8/8 files, 7 pure prompt bodies each (56 total). `build.md` is covered through the create/edit prompts rather than a separate prompt file. |
...
| Engine-companion roadmap | Live introspection re-opened three integration gaps: **namespace parity** (`_exec_dashboard_script` injects `pull_nyfed_data` / `pd` / `np`; `refresh_runner._build_exec_namespace` does not), **on-demand manifest-pointer update** (hourly updates it; Django refresh does not), and **duplicate runner import diagnostics**. Existing engine work remains in `projects/echarts/dev/notes.md`: typed script-edit transaction, popup canvas parity, pipeline-integrity companion, and remaining validator/catalog drift. |
