...
│                                       PRODUCTION REBASE LANDED      │
│                                       (2026-07-10): canonical       │
│                                       payload aligned to current   │
│                                       prism-main semantics; local  │
│                                       harness now uses core /      │
│                                       dashboards / jobs /          │
│                                       prism_mcp / prism_meta / web │
│                                       package trees directly; old  │
│                                       ai_development mirror        │
│                                       archived and inactive;       │
│                                       canonical_showcase.json      │
│                                       frozen for phase-two         │
...
│                                       pre-existing + 1 frozen-     │
│                                       showcase expected failure.   │
...
| Aspect | Value |
|---|---|
| Drag-and-drop status | prism-main production rebase LANDED locally (2026-07-10): verified production runtime/context deltas are in the canonical payload, and the mock harness now uses the split-package topology directly. The local production-oracle closure gate passes across all 7 Python and 9 Markdown overlap files. Candidate promotion remains external; the current promotion excludes `dashboards/canonical_showcase.json` and staging-only `test_prompts/`. |
| Canonical payload | `projects/echarts/echarts-payload/` — `__init__.py`, `config.py`, `dashboards_time.py`, `echart_dashboard.py`, `echart_studio.py`, `rendering.py`, `refresh_runner.py`, `refresh_dashboards.py`, `dashboards.md`, `dashboards_hub.md`, and `dashboards/*.md`. Python files land in `prism-core/dashboards/` except `refresh_dashboards.py`, which lands in parent-repo `jobs/hourly/`; context files land in `prism-core/context/modules/static/tools/`. `dashboards/canonical_showcase.json` is canonical staging content but frozen outside this promotion phase. `test_prompts/` is always staging-only. |
