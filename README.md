...
│  docstrings   ████████░░░░░░░░░░░░  9/11 intake landed (workshop    │
│                                       next; 2 modules still need a  │
│                                       source)                       │
...
| docstrings | 9/11 placeholder slots intake-landed (workshop next) | `projects/docstrings/` | `projects/docstrings/docstrings-payload/{*.py, *.md}` | L1 tool docstrings in `mcp/tools/{context_tool,global_tools,data_tools}.py` + L2 Tier 1 always-on static modules in `context/modules/static/{core,parsing_issue,code_sandbox_context,search_indexes,directory_tree,security_and_status,macro_style_guide,session_hygiene}.md` | — | `mcp-tools.md` §3, `architecture.md` §3.1, §3.3 | sourced from `projects/docstrings/dev/scans/2026-05-02_l1_l2t1_intake.md` (3/3 L1 + 6/8 L2-T1 + 2 bonus). Next: INQUIRY pass (per `prism-freshness.mdc`) → VERBATIM pass (overwrite placeholders + scaffold 3 new files). |
...
| echarts | per-spoke | 5 (`charts`, `widgets`, `widget_tool`, `filters`, `recipes`) — files seeded, bodies awaiting first round (best seeded by the 5 incidents in `projects/echarts/dev/feedback/`, per the README's "Feedback queue" section) |
| docstrings | per-unit (3 L1 + 3 L2-T1, +session_hygiene if scaffolded) | 6 today (`get_context`, `global_context`, `data_context`, `core`, `parsing_issue`, `macro_style_guide`); will grow to 9 as `code_sandbox_context`, `security_and_status`, `session_hygiene` get scaffolded next session per the docstrings-side README |
...
| Test prompts | `echarts-payload/test_prompts/{charts,widgets,widget_tool,filters,recipes}_test.md` — one per spoke, 7 prompts each. Per the cross-project convention. STAGING-ONLY (does NOT ship). Files seeded but currently empty; the 6 dashboard feedback scans (see "Feedback queue" below) are the natural seed for the first round of prompts. Prioritise prompts that exercise the new manifest-preservation rule (§2.5.4) so the round-trip verifies the fix. |
...
| Aspect | Value |
|---|---|
| Status | INTAKE LANDED. The 2026-05-02 OCR scan (filed at `projects/docstrings/dev/scans/2026-05-02_l1_l2t1_intake.md`, 2,714 lines) covers 9/11 placeholder slots. INQUIRY + VERBATIM + WORKSHOP passes are the active work surface from here. |
...
| L1 files | `get_context.py`, `global_context.py`, `data_context.py` — each carries a module-level `DOCSTRING: str` constant. **All 3 covered by intake** (lines 1-313 / 314-602 / 603-1027 of the intake scan). |
...
| L2 Tier 1 files (8 total — was 7, +session_hygiene 2026-05-02) | Scaffolded today: `core.md`, `parsing_issue.md`, `macro_style_guide.md` (the seed triplet) — all 3 covered by intake. To be scaffolded next session: `code_sandbox_context.md`, `security_and_status.md`, `session_hygiene.md` — all 3 also have intake content available. Still need a separate source: `search_indexes.md` (cached in S3, may have its own cadence), `directory_tree.md` (auto-generated from `ai_development/` tree, may not need a scanned source). `user_context` is also Tier 1 always-on but is a runtime module and out of scope for this project. |
...
| Intake coverage (9/11) | L1: 3/3 (get_context, global_context, data_context). L2 T1: 6/8 (core, parsing_issue, code_sandbox_context, security_and_status, macro_style_guide, session_hygiene). Bonus (Tier 2, out of scope but adjacent): end_user_abstraction (lines 1921-2030; lives in PRISM's `core/` composite), output_format_guide (2162-2323; subset of parsing_issue per the catalog). |
...
| Next step | (1) SCAFFOLD the 3 newly-coverable L2 T1 placeholders (`code_sandbox_context.md`, `security_and_status.md`, `session_hygiene.md`); (2) INQUIRY pass on the intake scan per `.cursor/rules/prism-freshness.mdc` (likely surfaces include the COMPLETE MODULE CATALOG, Specializations / Coalition & Inquiry / Prediction Markets Awareness, Session Folder Hygiene module — possible new `prism/` surfaces); (3) VERBATIM pass — overwrite all 3 L1 + 6 L2 T1 placeholders from intake; (4) WORKSHOP pass(es). No `.cursor/rules/docstrings.mdc` yet — same discipline as apis (wait for the pattern to prove itself before codifying). |
