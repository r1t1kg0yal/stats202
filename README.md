...
|---------|----------|-----------------------|----------------|-------------------|------|-------------|------------------------|
| altair | mature engine + markdown-table ban + lean registered router/core and 6 on-demand spokes; engine Python prod-verified, context rewrite pending promotion. API surface simplified 2026-07-25 (engine 28,057 -> 26,365 lines; one canonical placement per concept) | `projects/altair/` | `projects/altair/altair-payload/` | `prism-core/prism_mcp/utils/{chart_functions,chart_functions_studio}.py` + `prism-core/context/modules/static/tools/chart_context.md` (registered router/core) + `prism-core/context/modules/static/tools/charts/chart_context_{annotations,dual_axis,composites,tables,grids,colors}.md` | `.cursor/rules/viz-platforms.mdc` | `code-sandbox.md`, `mcp-utils.md`, `vision-qc.md`, `mcp-tools.md` §3+§5 | Re-promote engine + hub + composites/grids spokes together (API simplification changed both sides); then constrained RBR + PRISM promotion verify routing |
| altair2 | staging port of PRISM branch `macdist-refactor-chartRender`: the engine is extracted into an import-closed `prism_mcp/chart_render/` package (`core.py` + `house_style.py`, plus PRISM-owned `units.py`) and the old `utils/chart_functions.py` path becomes a 42-line trusted-side wrapper that injects presign / studio / vision / error-mail / font-root via `register_trusted_extensions()`. Built from `staging/altair-payload-aug2/` (the snapshot the branch was cut against) with the 18 documented `core.py` blocks applied. `house_style.py` reproduces the branch blob byte-exactly (24,348 B, `828e5873`); `core.py` parity is open because aug2 differs from the branch merge-base by ~5,734 bytes. Verified: AST closure gate, identical 44-entry `__all__`, and byte-identical PNGs across 8 chart types between the trusted and bare-sandbox paths. Exists alongside `projects/altair/` until signed off, then replaces it. | `projects/altair2/` | `projects/altair2/altair-payload/` (now TWO destination dirs: `chart_render/` + `utils/`) | `prism-core/prism_mcp/chart_render/{__init__,core,house_style}.py` + `prism-core/prism_mcp/utils/{chart_functions,chart_functions_studio,chart_functions_studio_tables}.py` + the 7 unchanged `chart_context*.md` | `.cursor/rules/viz-platforms.mdc` | `code-sandbox.md`, `mcp-utils.md`, `mcp-tools.md` §3+§5 | Byte-diff `core.py` against the branch; then `dev/_port_from_aug2.py --run --from-canonical` to forward-port the Aug 2-3 work (band / contribution + newer studio), which clears 38 of the 40 currently-failing tests |
...
| Tests | `projects/altair/dev/tests.py` (`python tests.py` interactive; `python tests.py unit -v` headless). API-only suite (109 tests) through `make_chart`, `make_table`, and composite helpers; no internal-function or engine-introspection drift gates. Paired with `dev/_probe_api_surface_contract.py run` (25 cases — every retired name rejects naming its canonical replacement) and `dev/_migrate_title_kwargs.py verify` (no call site passes a retired axis-title kwarg). |
