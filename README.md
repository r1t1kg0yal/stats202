...
> `fec2ace`). Every "gitlink vs checkout" parity blocker in the rows
...
| echarts | mature reliability-first compiler with Dashboard Garbage Gate, persisted input, refresh, and hub/spoke context; 2026-08-21 retrieval became declarative — `scripts/pulls.json` replaces `scripts/pull_data.py`, the engine owns the `get_data` call, and `build.py` is the only persisted Python left (PRISM-side status of the equivalent MR unverified; open prompt `2026-08-21_pulls_document.md`); 2026-08-20 live Composer state is owner-only response mount + all-12 allowlist + 10-of-12 header grips + seven-field canonical-only drag, with installed snapshot producer uncalled and validator/tests absent; 17 Composer endpoints; neutral reload-hold pair installed; persisted-input routes are live; installed-byte parity has not been re-measured since the 2026-08-07 vendoring — the old "checkout ahead of gitlink" blocker is void, so this is now a direct one-repo comparison waiting to be run | `projects/echarts/` | `projects/echarts/echarts-payload/` (12 Python files incl. `dashboard_share.py` + `dashboard_user_input.py` + `chrome_render.py` + `refresh_launcher.py` + 12 context files: registered router, on-demand kernel, 10 spokes; 8-file/63-prompt `test_prompts/` remains staging-only) | 11 canonical modules target `prism-core/dashboards/` + parent-tree `jobs/hourly/refresh_dashboards.py` + `prism-core/context/modules/static/tools/{dashboards,dashboards_hub}.md` + `tools/dashboards/*.md`; live extraction confirms `dashboard_share.py` and `dashboard_user_input.py` exist but did not rerun a complete payload inventory count; local runtime mirrors the vendored repo through `core/`, `dashboards/`, `jobs/`, `prism_mcp/`, `prism_meta/`, and `web/` | `.cursor/rules/viz-platforms.mdc` | `codebase-tree.md` §2+§9, `code-sandbox.md` §2.4+§8, `context-system.md` §5, `dashboard-refresh.md`, `dashboards-portal.md` | `projects/echarts/dev/specs/{composer_dashboard_stack,dashboard_user_input}.md`; live extraction archived at `staging/prompts/archive/2026-08-19_composer_dashboard_delta.md` |
...
| Aspect | Value |
|---|---|
| Drag-and-drop status | **IN SYNC — confirmed by direct inspection 2026-08-09.** The installed chart engine and chart context are identical to `projects/altair/altair-payload/`. Production runs the split: the `chart_render` refactor merged to `master` via `a9cd506` (2026-08-08), so the pre-split flat layout archived at `projects/altair/dev/archive/2026-08-08_pre_chart_render/` is history and the branch ref `macdist-refactor-chartRender` is stale (tip `97e9b441` is not an ancestor of HEAD; the content arrived through master). The later Ritik-authored commits on these paths — `5ff6e3f` (2026-08-08, two new chart types) and `a1620b6` (2026-08-09, chart functions improvements) — are promotions landing in PRISM's history, not PRISM-side edits. The `check_charts_quality` install hazard is void: `72fb925` retired Gemini chart QC entirely and `script_exec_tools.py` no longer imports the name. |
| Canonical payload | `projects/altair/altair-payload/` in four subfolders that mirror four PRISM destinations: `chart_render/{__init__,core,house_style}.py` -> `prism-core/prism_mcp/chart_render/`; `utils/{chart_functions,chart_functions_studio,chart_functions_studio_tables,vega_node_check}.py` -> `prism-core/prism_mcp/utils/`; `tools/chart_exec.py` -> `prism-core/prism_mcp/tools/`; and `core/chart_agent_tool.py` -> the prism-main parent tree's `core/`. The last two were adopted 2026-08-20 from screenshots, are not byte-verified, and do not ship yet. Plus the 7 flat `chart_context*.md`. `chart_render/units.py` is PRISM-owned (a pure `git mv` we only stub) and does not ship from here. Payload `.py` files carry no trailing newline. |
| Stub mirror | `projects/altair/{prism_mcp,prism_meta,core,web}/` — stub packages mirroring prism-main's import surface (`prism_mcp/chart_render/` shim + PRISM-owned `units.py` stub, 12 modules under `prism_mcp/utils/` — of which four are forwarders to `altair-payload/utils/` rather than stubs: `chart_functions`, both studios, and `vega_node_check` — plus `prism_mcp/tools/`, the `prism_meta.REPO_ROOT` anchor, `core.s3_bucket_manager`, and the fonts dir). The `chart_render/__init__.py` shim prepends the payload dir to `__path__` so `core` and `house_style` resolve to the payload while `units` resolves to the stub. Pre-prism-main trees archived at `dev/archive/2026-07-07_pre_prism_main_stubs/`; pre-split payload and stubs at `dev/archive/2026-08-08_pre_chart_render/`. |
| Pinned interpreter | The repo-root `.venv/` (Python 3.11.15) — shared by every project since the 2026-08-27 consolidation. From `projects/altair/` that is `../../.venv/bin/python`. It carries PRISM's five golden chart pins (`altair 4.2.2`, `numpy 1.26.4`, `pandas 2.0.3`, `pillow 12.2.0`, `vl-convert-python 1.9.0.post1`); `pandas 2.0.3` ships no wheel past cp311, which is what fixes the interpreter at 3.11. Defined by the root `requirements.txt` / `requirements.lock.txt`. |
...
| Aspect | Value |
|---|---|
| Drag-and-drop status | The payload is 12 Python modules. Ten were confirmed installed alongside the router, kernel, and spokes; `dashboard_share.py` and `dashboard_user_input.py` both exist under `prism-core/dashboards/`. `chrome_render.py` and `refresh_launcher.py` came the other way — extracted by a `prism-core` MR on 2026-08-23 and applied here so both sides converge — and their install state has not been re-measured. **The gitlink parity blocker is gone** — `prism-core` is no longer a submodule (vendored by `287311b`, 2026-08-07), so there is no parent-recorded SHA to be behind and parity is a direct one-repo measurement. Parity has not been re-verified since the vendoring and remains an open measurement — unlike altair, which was confirmed in sync on 2026-08-09. |
| Canonical payload | `projects/echarts/echarts-payload/` — `__init__.py`, `config.py`, `chrome_render.py`, `dashboards_time.py`, `echart_dashboard.py`, `echart_studio.py`, `rendering.py`, `dashboard_share.py`, `dashboard_user_input.py`, `refresh_launcher.py`, `refresh_runner.py`, `refresh_dashboards.py`, `dashboards.md`, `dashboards_hub.md`, `dashboards/*.md`, and since 2026-08-22 the Composer consumer leaf `web/js/dashboard_composer.js` + `web/news/dashboard_composer.py`. Python files land in `prism-core/dashboards/` except `refresh_dashboards.py` → parent-repo `jobs/hourly/`, `web/js/dashboard_composer.js` → parent-repo `web/prism_site/js/`, and `web/news/dashboard_composer.py` → parent-repo `web/backend_django/news/`; context files land in `prism-core/context/modules/static/tools/`. The `web/` pair is subfoldered by destination precisely because it lands in the parent tree rather than `prism-core/`, and it is the one place the Composer-literal ban does not apply — that ban is scoped to the neutral producer (`rendering.py` and its `prism-core/dashboards/` siblings). `test_prompts/` is always staging-only. |
...
| Validation gates | Strict compilation reports all structural, compute, binding, and data diagnostics in one pass; always-blocking render failures raise even with `strict=False`. The Dashboard Garbage Gate adds a bounded one-line-per-panel review, flagged-panel drill-down, exact rationale acknowledgment, signature-stable raw refreshes, and the same current-data gate for saved-definition restores. Literal date tokens and invalid gauges block; sparse/gappy/spiky lines, categorical `ALL_ZERO`/`PARTIAL` marks, narrative-table wrap risk, and browser-only tools require review. Pull stems are read off `scripts/pulls.json`, so the pull half of producer analysis is gone — an unresolved output is now only a transform concern, and the document itself is validated against the `get_data` `Details` union before it can be written. Transform analysis still follows local materializer helpers, literal propagation, and finite literal loops while separating unresolved dynamic outputs from definite stale/unattached datasets. |
...
| venv | The repo-root `.venv/` (Python 3.11.15), shared with every other project since the 2026-08-27 consolidation — invoke as `../../.venv/bin/python` from `projects/frontend/`. Supplies Django, Flask, requests, markdown, PyYAML and playwright (dev-only, for screenshot QC). Add packages to the root `requirements.txt`; do not create a project-local venv. |
...
| `altair-payload/` | Ephemeral drag-and-drop copy of `projects/altair/altair-payload/`. User refreshes before promoting to PRISM. Post-`chart_render` split, so it carries the `chart_render/` + `utils/` subfolders. Historical snapshots (`altair-payload-aug1/` through `aug9/`) were removed 2026-08-19; pre-split and pre-consolidation states live under `projects/altair/dev/archive/`. |
