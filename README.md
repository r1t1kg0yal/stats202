...
| echarts | mature + time/live-refresh uplift landed (2026-05-12); prism-main production rebase + split-package local harness landed (2026-07-10); awaits promotion classifier | `projects/echarts/` | `projects/echarts/echarts-payload/` (8 Python files, preflight + authoring hub, and 7 Markdown spokes; `test_prompts/` remains staging-only) | `prism-core/dashboards/*.py` + parent-tree `jobs/hourly/refresh_dashboards.py` + `prism-core/context/modules/static/tools/{dashboards,dashboards_hub}.md` + `tools/dashboards/*.md`; local runtime mirrors the split checkout through `core/`, `dashboards/`, `jobs/`, `prism_mcp/`, `prism_meta/`, and `web/` | `.cursor/rules/viz-platforms.mdc` | `dashboard-refresh.md`, `dashboards-portal.md` | `staging/prompts/open/2026-05-11_views_py_time_refresh_uplift.md`; follow-ups: `2026-05-12_refresh_dashboards_daemon_install.md`, `2026-05-12_pull_helpers_sidecar.md`, `2026-05-12_pull_market_data_tz_normalization.md` |
...
| Aspect | Value |
|---|---|
| Drag-and-drop status | prism-main production rebase LANDED locally (2026-07-10): verified production runtime/context deltas are in the canonical payload, and the mock harness now uses the split-package topology directly. The local production-oracle closure gate passes across all 7 Python and 9 Markdown overlap files. Candidate promotion remains external; `test_prompts/` is staging-only. |
