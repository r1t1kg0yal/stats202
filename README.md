...
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  altair       ████████████████████  mature + TABLES SURFACE landed   │
│                                       (2026-05-12): make_table()      │
│                                       static-PNG table engine in      │
│                                       chart_functions.py; new spoke   │
│                                       chart_context_tables.md (hub +  │
│                                       2 spokes total); 3 PRISM-facing │
│                                       color modes ('rwg' / 'bw' /     │
│                                       'rag'); heatmap_groups (col /   │
...
│                                       additive correction).        │
│  bloomberg    ███░░░░░░░░░░░░░░░░░  scaffolded 2026-05-15 — 1 skill │
│                                       file (~940 lines) covering   │
│                                       full BQL + DAPI catalog,     │
│                                       openpyxl gotchas, ~200 field │
│                                       mnemonics, awaits first      │
│                                       PRISM round-trip             │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```
...
| bloomberg | scaffolded 2026-05-15 — single skill file, comprehensive on first pass; awaits PRISM round-trip | `projects/bloomberg/` | `projects/bloomberg/bloomberg-payload/bloomberg_excel.md` | `ai_development/context/modules/static/tools/bloomberg_excel.md` (+ `context/registry.py` entry) | — | — | `projects/bloomberg/bloomberg-payload/bloomberg_excel.md` IS the spec. ~940 lines covering: openpyxl gotchas (`_xll.` prefix, `ArrayFormula`, double-quote escaping), DAPI vs BQL decision table, security syntax (yellow keys + identifier types), full DAPI surface (BDP/BDH/BDS/BEQS/BCURVE/BSRCH + override flag catalog), full BQL surface (BQL.Query / BQL.Dates / BQL.Params / BQL.Expr + universe builders + aggregation/group + statistical/technical functions + field-level + with-clause parameters), curated field mnemonic catalog by asset class (~200 mnemonics across equity prices/identifiers/fundamentals + fixed income yields/spreads/duration/ratings + FX + commodities + indices), openpyxl integration patterns (4 worked examples), anti-patterns table, quick-reference cheat sheet. Workshop pass + RBR (roleplay-before-round-trip) deferred until first PRISM round-trip surfaces concrete frictions. |
...
| Aspect | Value |
|---|---|
| Status | Scaffolded 2026-05-15. One skill file (~940 lines) on first pass. No dev/, no demos, no stub mirror — the payload IS the file IS the spec. Awaits first PRISM round-trip. |
| Canonical payload | `projects/bloomberg/bloomberg-payload/bloomberg_excel.md` — flat single-file payload (whitepapers-style) |
| PRISM destination | `ai_development/context/modules/static/tools/bloomberg_excel.md` + one entry in `context/registry.py` (key `bloomberg_excel`, pillar `tools`, content_type `static`) |
...
| Drop-in mapping | `projects/bloomberg/README.md` documents it. Single file, byte-identical drag-and-drop. Promote = copy + add registry entry. |
...
| Next step | (a) User reviews the skill content; (b) drop into PRISM at `ai_development/context/modules/static/tools/bloomberg_excel.md` + add registry entry; (c) end-usage round-trip — user prompts PRISM "build me an Excel workbook that pulls SPX members and their P/E and 200-day momentum" or similar; (d) frictions surfaced in PRISM's reply (kwarg guesses, missing fields, wrong override flag, sizing mistakes) drive a workshop pass HERE; (e) iterate. RBR (roleplay-before-round-trip) optional given the simple shape. No `.cursor/rules/bloomberg.mdc` yet — same wait-for-pattern discipline as whitepapers / apis. |
