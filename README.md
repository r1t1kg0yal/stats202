...
| Test prompts | `altair-payload/test_prompts/{chart_types,mapping,annotations,dual_axis,composites}_test.md` — one per topic area, 7 prompts each. STAGING-ONLY (does NOT ship); independent of the router/spoke packaging. |
...
| Remaining integration work | Browser-triggered refresh still does not update the user-manifest dashboard pointer; the hourly orchestrator does. Production promotion also remains blocked until the live `prism-core` checkout is reconciled to the commit recorded by `prism-main` and candidate-versus-live byte classification is rerun. Composer Stages 1–2b are landed in the PRISM working tree (2026-07-19 review): `dashboard_composer` injector with `PRISM_COMPOSER_DND_MODE=dashboard_components`, owner-only component drag, inlined widget bodies via `composer_prompt.py`, live resolver, multi-tab/history/SSE polish. Staging `rendering.py` already has widgets + `prism:dashboard:ready` + brand-home — promote before claiming parity. SSOT: `projects/echarts/dev/specs/composer_dashboard_stack.md`. Remaining Composer hygiene: confirm `enable_inline_chat`, strip local-dev port/ACL toggles, empty `website_dev.md` registry follow-up. The reliability roadmap's namespace, strict-mode, data-integrity, helper-aware producer analysis, transactional-script, popup/filter/layout, theme, geo/grid, and export work is complete locally. |
