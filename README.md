...
| echarts | mature + time/live-refresh uplift landed (2026-05-12); per-dashboard cadence + daemon mode + tz-mix engine absorption landed (2026-05-12 follow-up); awaits PRISM-side handoff | `projects/echarts/` | `projects/echarts/echarts-payload/` (engine + runner files + the new `dashboards_time.py` canonical time helpers; `parse_freq` accepts "60s"/"5m"/"1h"/"1d"/"1w" + legacy enum; `refresh_dashboards.py --interval N` daemon mode) | `ai_development/dashboards/*.py` (engine + runner + `dashboards_time`) + `context/modules/static/tools/dashboards.md` (hub) + `dashboards/*.md` (7 spokes incl. template_crud.md) | `.cursor/rules/viz-platforms.mdc` | `dashboard-refresh.md`, `dashboards-portal.md` | `staging/prompts/open/2026-05-11_views_py_time_refresh_uplift.md` (the PRISM-side half: NEW dashboard_data_api endpoint + template_hash injection + refresh_status_api auto-heal rewrite + views.py datetime-parse audit). Follow-ups: `staging/prompts/open/2026-05-12_pull_helpers_sidecar.md` (pull_completed_at stamping — observability-only); `staging/prompts/open/2026-05-12_pull_market_data_tz_normalization.md` (uniform tz convention across pull_market_data modes — source-side hygiene). |
...
| Canonical payload | `projects/altair/altair-payload/chart_functions.py`, `chart_functions_studio.py`, `chart_context.md` (hub) + `chart_context_grids.md` (1 spoke for grid / small-multiples) |
...
| Skill shape | Hub + 1 spoke. `chart_context.md` (hub, ~600 lines after the 2026-05-10 Chart Center disable; ~665 lines pre-disable) carries the always-needed surface — namespace + QC + design defaults + authoring rules + chart types + mapping + annotations + dual-axis + composites + dimensions + horizons + failure transparency. `chart_context_grids.md` (spoke, ~157 lines) carves off the grid / small-multiples topic — `mapping['facet']`, `same_scale` smart-route, scatter phase-space gradient, 36-panel hard cap — fetched mid-session via `list_ai_repo(file_paths=["context/modules/static/chart_context_grids.md"], mode="full")`. `TestChartContextCoverage` in `dev/tests.py` pins skill/engine drift on the hub. The broader hub-and-spoke split landed 2026-05-02 and was reverted same-week in favor of single-file ergonomics; the 2026-05-09 grid spoke is the carve-out exception (genuine standalone topic + 36-panel discipline + scatter color-gradient affordance that doesn't apply elsewhere). The 2026-05-09 consolidation also folded the user's `chart_context_rewrite.md` compression draft into the canonical (deleted; net -350 lines) and archived `chart_context_v2.md` to `dev/archive/`. The 2026-05-10 Chart Center disable removed the §11 surface entirely (catalog rows, ChartResult `editor_*` table rows, QC example block, full §11 Prism Chart Center body); shallow-disable retains the dataclass `editor_html_path` / `editor_download_url` / `editor_chart_id` fields as zombie Optional[None] for easy re-enablement. |
...
| Canonical payload | `projects/echarts/echarts-payload/` — `__init__.py` (re-exports the new entry points), `config.py`, `echart_dashboard.py` (engine + the new `run_pull` / `build_dashboard` / `refresh_dashboard` helpers), `echart_studio.py`, `rendering.py`, **`refresh_runner.py` (NEW v2)**, **`refresh_dashboards.py` (NEW v2)**, `dashboards.md` (hub, ~94 KB) + `dashboards/*.md` (7 spokes incl. `template_crud.md` thinned) + `dashboards/canonical_showcase.json` (templated `build_showcase` reference manifest, ~111 KB). The `*.json` glob is part of the drag-and-drop contract per `.cursor/rules/viz-platforms.mdc`. |
| Static-asset mirror | `projects/echarts/ai_development/mysite/news/static/js/echarts.js` (no Python stub mirror needed — stdlib + pandas only; v2 runner files import from canonical PRISM paths and are PRISM-only — see `projects/echarts/ai_development/README.md`) |
