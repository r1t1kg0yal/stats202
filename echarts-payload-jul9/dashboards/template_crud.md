...
1. **AUDIT** -- confirm the dashboard folder is canonical (`dashboards_hub.md` §2.5.3); raises if not, in which case realignment takes priority over the surface change
...
`width_sum` matters because chart widgets must be `w=6` or `w=4` (`dashboards_hub.md` §4.1) and a row's widths sum to ≤ `cols` (default 12). Adding a chart to a row that's already at `w_sum=12` requires a new row, not appending to the existing one.
...
Three required fields per `dashboards_hub.md` §2.3 -- `kerberos`, `dashboard_id`, `methodology` -- must be non-empty for the validator to pass. If the inherited template has any of these missing, set them BEFORE the rest of the mutation:
...
if not md.get("methodology"):
    md["methodology"] = (
        ...
        "## Construction\n"
        "* Daily close pulled via pull_plottool_data; no transforms"
    )
```
...
`header_actions[]` injects custom buttons to the LEFT of the always-on chrome (Methodology / Refresh / Share / Download). Reserved chrome ids cannot collide; the validator hard-rejects collisions. See `dashboards_hub.md` §5 for the full reserved list.
...
# its first line per dashboards_hub.md Rule 5 (the literal is baked into
...
**This is the in-session loop.** PRISM iterates: CRUD → write → recompile → see error → CRUD again. Tool 4's subprocess refresh (`dashboards_hub.md` §6.1 Tool 4) is the canonical end-of-edit verification -- it runs the SAME `build.py` plus a fresh `pull_data.py` re-exec in a clean interpreter, and IS load-bearing before surfacing the portal URL to the user.
...
| Tool 4 subprocess refresh (`dashboards_hub.md` §6.1) | Once at end-of-edit | Tomorrow's cron will produce byte-identical output; pull pipeline still works |
...
|---|---|---|
| 1 | AUDIT before mutating (`dashboards_hub.md` §2.5.3) | Edits land on a non-compliant folder; runner picks up wrong bytes |
...
Rule 5 is the manifest-wipe footgun (`dashboards_hub.md` §2.5.4). The CRUD patterns above are the operational answer -- they MUTATE in place, never `tpl = {...}` from a fresh dict.
