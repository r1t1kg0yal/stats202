...
`show_values`, `value_decimals`, `value_formatter`, `value_label_color`, `value_label_size`, `colors`, `color_palette`, `color_scale`, `value_min`, `value_max`, `hatch_missing`, `size_by`
...
### Weighting cells by a second column

`size_by` names a second numeric column drawn as cell **area** while `value`
stays the colour — the matrix answer to "this cell is red, but is it worth
anything?" (a wide spread on $2mm notional next to a thin one on $400mm).

```python
{"chart_type": "heatmap", "dataset": "book",
 "mapping": {"x": "tenor", "y": "sector", "value": "spread_change",
             "size_by": "notional_usd"}}
```

Area, not edge length, is proportional to the value, and the largest cell in
view sets the top of the scale — so a filter renormalises against what is
left rather than against a maximum now off screen. Both numbers, named, are
in the cell tooltip; the colour bar speaks only for `value`.

Constraints, all blocking:

- `show_values` and `size_by` are mutually exclusive. A cell scaled to a
  fraction of its slot is narrower than the number it would carry, so the
  text spills into the neighbour and reads as belonging to it. Use the
  tooltip.
- No negative sizes — area has no sign. Take an `abs()` in a computed column
  and put the direction in `value`.
- Every drawn cell needs a size. A null one would render at minimum area and
  read as "nearly zero" rather than "not known". Rows whose `value` is null
  draw no cell at all, so their size is moot and may be null.

An all-zero `size_by` column is a warning, not an error: every cell floors at
the same minimum area and the encoding says nothing.
...
### Relationship kinds on a graph

`node_category` colours the nodes; `edge_class` names a column on the edge
rows that says what *kind* of relationship each edge is — `guarantees` versus
`supplies` versus `lends to`, which read as the same line without it.

```python
{"chart_type": "graph", "dataset": "exposures",
 "mapping": {"source": "from_entity", "target": "to_entity",
             "value": "exposure_mm", "node_category": "sector",
             "edge_class": "relationship"}}
```

Each class gets a palette colour **and** a dash pattern, assigned in
first-appearance order so a rebuild on the same data restyles nothing. The
dash is not decoration: hue alone fails the colour-blind reader and dies in a
printed page. ECharts binds the legend to node `categories`, so the class name
also goes into the edge tooltip — a colour encoding with no readable key is
decoration rather than information. A typo in `edge_class` is column-checked
and blocks, rather than quietly rendering every edge in one style.

The force layout settles in one pass and the view is then scaled onto whatever
it produced, so the whole network is on canvas whether it is 8 nodes or 80 —
nothing to tune, and no silently-cropped subgraph. A dense frame therefore
draws smaller rather than partly off-tile; if the nodes come out too small to
read, that is the frame asking for a filter or a `top_n`, not a layout knob.
