...
# renderItem for a ``size_by`` heatmap cell. Pulls the cell extent from
# ``api.size`` rather than a baked constant, so the rect stays correct
# through a window resize and through a filter that changes the category
# count. The area fraction arrives pre-computed as dimension 4 -- the
# normalisation needs the whole column's maximum, which renderItem
# cannot see, and keeping it in the data means a client-side filter
# rewire only has to redo arithmetic.
HEATMAP_SIZED_RENDER_JS: str = (
    "function(params, api){"
    "var f=api.value(4);"
    "if(f==null||isNaN(f)||f<=0){return;}"
    "var pt=api.coord([api.value(0),api.value(1)]);"
    "var cell=api.size([1,1]);"
    "var w=cell[0]*f, h=cell[1]*f;"
    "return {type:'rect',"
    "shape:{x:pt[0]-w/2,y:pt[1]-h/2,width:w,height:h},"
    "style:api.style()};"
    "}"
)


def heatmap_sized_tooltip(
    x_cats: Sequence[Any], y_cats: Sequence[Any],
    value_name: str, size_name: str,
) -> str:
    """Tooltip formatter for a ``size_by`` cell: both encodings, named.

    Area is judged badly by eye and the colour bar only speaks for
    ``value``, so the tooltip is the only place the second number is
    readable -- which is also why the sized series refuses cell labels.
    Category names are baked in against the compile-time axes; a
    dashboard filter rewire moves index positions and rebuilds this
    with the surviving categories (see ``_sizedHeatmapTooltip``).
    """
    return (
        "function(p){var v=(p.data&&p.data.value)||p.data||[];"
        "var xs=" + json.dumps([str(c) for c in x_cats]) + ";"
        "var ys=" + json.dumps([str(c) for c in y_cats]) + ";"
        "return (ys[v[1]]||'')+' / '+(xs[v[0]]||'')+'<br/>'"
        "+" + json.dumps(str(value_name)) + "+': '+v[2]+'<br/>'"
        "+" + json.dumps(str(size_name)) + "+': '+v[3];}"
    )


def heatmap_size_fractions(sizes: Sequence[Any]) -> List[Optional[float]]:
    """Area fractions for a ``size_by`` column, largest cell at the cap.

    Area -- not edge length -- is proportional to the value, hence the
    square root: encoding magnitude on the edge would overstate a large
    cell by the square of its true ratio.
    """
    finite = [abs(float(s)) for s in sizes if _is_finite(s)]
    top = max(finite) if finite else 0.0
    out: List[Optional[float]] = []
    for s in sizes:
        if not _is_finite(s) or top <= 0:
            out.append(None)
            continue
        frac = math.sqrt(abs(float(s)) / top) * HEATMAP_SIZE_MAX_FRAC
        out.append(max(HEATMAP_SIZE_MIN_FRAC, min(HEATMAP_SIZE_MAX_FRAC,
                                                    frac)))
    return out


def _heatmap_sized_series(
    *, name: str, data: List[Dict[str, Any]],
    x_cats: Sequence[Any], y_cats: Sequence[Any],
    value_name: str, size_name: str,
) -> Dict[str, Any]:
    """The value series for a ``size_by`` heatmap, as a custom rect series.

    An ECharts ``heatmap`` series always fills its whole cell, so a
    second magnitude encoding needs a series type with a free geometry
    dimension. ``custom`` gives that and, unlike a rect-symbol scatter
    with a baked ``symbolSize``, it reads the live cell extent from
    ``api.size`` -- so the matrix stays truthful when the tile is
    resized.
    """
    return {
        "name": name,
        "type": "custom",
        "renderItem": HEATMAP_SIZED_RENDER_JS,
        "encode": {"x": 0, "y": 1, "tooltip": [2, 3]},
        "data": data,
        "emphasis": {"itemStyle": {"shadowBlur": 6,
                                     "shadowColor": "rgba(0,0,0,0.3)"}},
        "tooltip": {"formatter": heatmap_sized_tooltip(
            x_cats, y_cats, value_name, size_name)},
    }
...
def build_heatmap(df, mapping: Dict[str, Any], ctx: BuilderContext) -> Dict[str, Any]:
    ...
    Mapping keys:
        ...
        size_by            second numeric column encoded as cell AREA,
                           on top of ``value`` as colour. Switches the
                           series to a custom rect series, since an
                           ECharts heatmap series always fills its cell.
                           Incompatible with ``show_values``.
    """
    ...
    size_by = mapping.get("size_by")
    required = [x, y, val] + ([size_by] if size_by else [])
    _ensure_columns(df, required, "heatmap")
    ...
    vals: List[Any] = []
    present: set = set()
    size_col = _col_to_list(df, size_by) if size_by else None
    for i, (xv, yv, z) in enumerate(zip(
        _col_to_list(df, x), _col_to_list(df, y), _col_to_list(df, val)
    )):
        if xv is None or yv is None:
            continue
        row = [x_idx[xv], y_idx[yv], z]
        if size_col is not None:
            row.append(size_col[i])
        cells.append(row)
        if z is not None:
            vals.append(z)
        if _is_finite(z):
    ...
    if size_by:
        # Colour must stay pinned to the value: a custom series' default
        # visualMap dimension is the last one in the item, which is now
        # the area fraction, and the two encodings would swap.
        opt["visualMap"][0]["dimension"] = 2
        fracs = heatmap_size_fractions([c[3] for c in cells])
        cell_data = [{"value": list(c) + [f]}
                     for c, f in zip(cells, fracs)]
        opt["series"] = [_heatmap_sized_series(
            name=str(val), data=cell_data,
            x_cats=x_cats, y_cats=y_cats,
            value_name=str(val), size_name=str(size_by),
        )]
        # Sidecar for the filter-rewire path, same reason as the hatch
        # one below: the tooltip names its cell by axis index, and a
        # filter renumbers the axes.
        opt["_heatmap_size_runtime"] = {
            "value_name": str(val), "size_name": str(size_by),
        }
    else:
...
def build_graph(df, mapping: Dict[str, Any], ctx: BuilderContext) -> Dict[str, Any]:
    ...
    edge_class = mapping.get("edge_class")
    ...
    if nodes is not None and edges is not None:
        ...
        if edge_class:
            _ensure_columns(df, [edge_class], "graph")
        ids = _unique(_col_to_list(df, src) + _col_to_list(df, tgt))
        node_list = [{"id": str(i), "name": str(i)} for i in ids]
        class_list = (_col_to_list(df, edge_class) if edge_class
                      else [None] * len(df))
        for a, b, vv, cc in zip(_col_to_list(df, src), _col_to_list(df, tgt),
    ...
    # Edge classes: one colour + one dash pattern per class, assigned in
    # first-appearance order so the same data yields the same styling on
    # every rebuild. Nodes own the legend (ECharts binds it to
    # ``categories``, which are node-side), so the class name also goes
    # into the edge tooltip -- a colour encoding with no readable key is
    # decoration, not information.
    edge_classes: List[str] = []
    if edge_class or any("edge_class" in e for e in edge_list
                          if isinstance(e, dict)):
        for e in edge_list:
            if not isinstance(e, dict):
                continue
            name = e.get("edge_class")
            if isinstance(name, str) and name and name not in edge_classes:
                edge_classes.append(name)
    if edge_classes:
        colors = (
            list(ctx.palette_colors)
            if ctx.palette_colors and ctx.palette_kind == "categorical"
            else palette_colors_safe("gs_primary") or []
        )
        # Hue alone fails for the colour-blind reader and in print, so
        # each class carries a dash pattern too.
        dashes = ["solid", "dashed", "dotted", [10, 5], [4, 4], [2, 2]]
        style_for = {
            name: {
                "color": colors[i % len(colors)] if colors else None,
                "type": dashes[i % len(dashes)],
            }
            for i, name in enumerate(edge_classes)
        }
        for e in edge_list:
            if not isinstance(e, dict):
                continue
            style = style_for.get(e.get("edge_class"))
            if style is None:
                continue
            line = dict(e.get("lineStyle") or {})
            if style["color"] is not None:
                line.setdefault("color", style["color"])
            line.setdefault("type", style["type"])
            # The series default (0.6 / 1.2px) is tuned for one
            # undifferentiated edge style; at that weight a dotted line
            # reads as a faint solid one.
            line.setdefault("opacity", 0.9)
            line.setdefault("width", 1.8)
            e["lineStyle"] = line
        opt["tooltip"]["formatter"] = (
            "function(p){"
            "var d=p&&p.data||{};"
            "if(p.dataType==='edge'){"
            "var cls=d.edge_class?"
            "('<br/><span style=\"opacity:.7\">'+d.edge_class+'</span>'):'';"
            "var v=(d.value==null)?'':('<br/>'+d.value);"
            "return d.source+' \\u2192 '+d.target+v+cls;"
            "}"
            "var nv=(d.value==null)?'':('<br/>'+d.value);"
            "return (p.name||d.name||'')+nv;"
            "}"
        )
    ...
    series = {
        ...
        "symbolSize": 28,
        # The settle is computed in one pass rather than animated over
        # frames: the runtime fit (`fitForceGraphView`) scales the view
        # to whatever the unbounded force layout produced, and it can
        # only measure a layout that has stopped moving. It also makes a
        # static render -- a PDF, an emailed PNG, a vision-QC
        # screenshot -- show the finished network instead of a frame of
        # the simulation.
        "force": {"repulsion": 800, "edgeLength": 120,
                    "gravity": 0.08, "layoutAnimation": False},
    ...
    if categories:
        ...
        # A graph has no grid, so the default layout box is the centred
        # 80% of the canvas -- which reaches up into the legend row at
        # y=42..62 and lets a node label land underneath the category
        # swatches. Starting the force area below that row in px (the
        # row is px-positioned) keeps the two apart at every tile
        # height; the 76% height leaves bottom slack down to ~270px.
        series["top"] = 64
        series["height"] = "76%"
    opt["series"] = [series]
    return opt
