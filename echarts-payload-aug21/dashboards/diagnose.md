...
| `graph.pipelines` | Declared pull names, each entry's `source`, and the CSV stems it commits to writing |
| `graph.csvs` | Persisted CSV stems and producer links |
| `graph.transforms` | Dataset keys materialized by `TRANSFORMS` |
| `graph.producer_analysis` | Transform output sites the registered call-graph analyzer cannot resolve; the pull list is always empty, since declared stems cannot be unresolved |
...
`transform_producer_output_unresolved` means a registered transform reaches an
...
There is no pull counterpart. Pull stems are stated in `scripts/pulls.json`,
so a declared pipeline is either readable or the document is refused — the
document-level codes are `pulls_missing` and `pulls_invalid`.

For an unresolved transform site, inspect `scripts.build.unresolved_outputs`
and `graph.producer_analysis.transform_unresolved`. Expose each fixed key at
the literal assignment/update, local materializer-helper call, or finite
literal loop; do not add no-op transform assignments. Only a definite
silent-stale/unattached finding justifies adding a missing producer.
...
The panel shows compile findings with severity and fix hint, declared datasets with live row counts, every widget with the datasets it reads and its vintage stamp, filters with their targets, and provenance (renderer version, data-as-of, review status, definition and review signatures). Dataset row counts are read from live state rather than the compile, so a refreshed dashboard does not describe itself with stale shapes. When the last cycle was `partial`, Provenance additionally names the stale sources from `metadata.time.stale_pulls`, so a reader who doubts one number can tell whether its source actually moved.
...
`apply_manifest_operations(state, operations)`. Retrieval heals use
`apply_pulls_document(state, document)`; derivation heals use
`apply_persisted_script_operations(state, "build", operations)`. Each
runs the affected pull for current-cycle schema verification, then calls
`launch_clean_refresh(FOLDER)`. Cadence heals use
`synchronize_refresh_frequency` with the inspected guards.

For any typed transaction, success proves the commit path when
