...
The five required paths are `manifest_template.json`, `manifest.json`, `dashboard.html`, `scripts/pulls.json`, and `scripts/build.py`. A dashboard that retrieves nothing still carries `{"schema_version": 1, "pulls": []}`, so "declares no pulls" never looks like "the document is missing". `audit_dashboard_layout(folder)` confirms presence and allows other legitimate files. Successful changed builds automatically version the template, the document and the script under `history/`; CSVs and generated outputs are never versioned.
...
| API | Contract |
|---|---|
| `run_pull(folder, pull_name)` | Execute one entry declared in `scripts/pulls.json` and persist its CSV |
...
Each declared entry runs in its own failure boundary, so one dead source does not cost the cycle:

| Pull outcomes | `refresh_status.status` | Build | Stamp |
|---|---|---|---|
| all succeeded | `success` | runs | any previous `stale_pulls` cleared |
| some failed | `partial` | runs | `metadata.time.stale_pulls` names the failed pulls |
| all failed | `error` | skipped | untouched; served bytes do not move |

A failed pull leaves its CSV at the previous cycle's bytes, so the build still has a full dataset set to compile against — which is exactly why the artifact has to say so. `metadata.time.stale_pulls` is engine-stamped, never authored, and turns the header freshness pill from `Refreshed <time>` into `Partial refresh <time>` with the stale source names on hover; the Diagnostics panel lists them under Provenance. `refresh_status` additionally carries `stale_pulls` and `fresh_pulls` for triage, and `partial` counts as unhealthy for the cron's failure cooldown.

Total pull failure is not a partial cycle. Nothing was re-read, so advancing `refresh_cycle_at` over unchanged data would claim a refresh that moved nothing — the runner raises instead and the previously served bytes stay.

### What produces a dataset

Retrieval is declared and derivation is coded, and the refresh-attachment
contract reads each from where it lives.

Pull stems come straight off `scripts/pulls.json`: the entry `name` is the
stem, except under `lakehouse` / `gs_quant_reference`, where each table's
literal `label` gives `<name>_<label>`. Nothing is inferred, so a pull
output can no longer be unresolved — an unlabelled table is refused at
write time instead.

Transform keys are still call-graph based. The engine starts from the
literal module-level `TRANSFORMS` in `scripts/build.py`, follows local
...
No pull helper is bound, so a persisted script that tries to fetch fails
on the name rather than opening a retrieval path the audit cannot see.
