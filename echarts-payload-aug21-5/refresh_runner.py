...
(``refresh_dashboards.py``). Runs ``run_pull`` for every entry declared
in ``<folder>/scripts/pulls.json``, then either a light data-only build
or a full ``build_dashboard``, then writes
``<folder>/refresh_status.json``.
...
Usage::
    ...
    python -m dashboards.refresh_runner \
        ...
        --pulls     rates swaps                              # optional subset
...
``classification`` pills and the ``log_path`` PRISM-triage field)::
    ...
    {
        "status":          "running" | "success" | "partial"
        ...
        "errors":          [{"script": "scripts/pulls.json::rates",
                              "classification": "data_pull_empty",
                              "message":   "...",
                              "traceback": "..."}],              # final only
        "stale_pulls":     ["rates"],      # when any pull failed
        "fresh_pulls":     ["equities"],   # when any pull failed
    }

Per-pull isolation: each declared entry runs in its own ``try``, so one
failing source does not abort the cycle. Outcomes fan into three
statuses:

* every pull succeeded -> ``success``, build runs normally.
* some failed, some succeeded -> ``partial``. The build still runs, so
  the sources that did land are current; the failed sources keep their
  previous CSV bytes. ``metadata.time.stale_pulls`` is stamped into the
  manifest so the artifact itself declares which datasets are carrying
  last-cycle data, and the header freshness pill switches from
  "Refreshed" to "Partial refresh" rather than claiming a clean cycle.
* every pull failed -> ``error``, and the build is skipped. Building
  would restamp the cycle clock onto data where nothing was re-read,
  which is the one outcome worse than not updating.
...
_PHASE_PULL_PREFIX = "scripts/pulls.json"
_PHASE_PULL_LOAD = "scripts/pulls.json::<load>"
...
def _list_pulls(folder: str, only: Optional[list] = None) -> list:
    """Read the declared pull names from ``<folder>/scripts/pulls.json``.

    Used by the runner to drive ``run_pull(folder, name)`` per pull so a
    per-pull failure surfaces the failing pull's name in the ``script``
    field of the \u00a78.1 ``errors[]`` entry.

    ``only`` selects a subset for a partial refresh, returned in the
    document's own order. An unknown name raises rather than being
    skipped: a caller asking for a pull that does not exist has a wrong
    idea of the dashboard, and silently refreshing nothing would look
    exactly like success.
    """
    declared = list(_load_pulls_spec(folder, s3_manager=s3_manager))
    if only is None:
        return declared
    unknown = [name for name in only if name not in declared]
    if unknown:
        raise RuntimeError(
            f"refresh_runner: --pulls named {unknown}, which "
            f"{folder}/scripts/pulls.json does not declare; "
            f"available: {declared}"
        )
    return [name for name in declared if name in set(only)]
...
def run(
    ...
    mode: str = "full",
    pulls_subset: Optional[list] = None,
) -> int:
    ...
    ``pulls_subset`` restricts phase 1 to the named pulls, in the
    document's own order, and the build still runs over every dataset.
    Useful when one source is slow or was the only one to fail; an
    undeclared name raises rather than silently refreshing nothing.
    """
    ...
    # Per-pull outcomes. Populated in PHASE 1; ``stale_pulls`` flows into
    # the manifest so the artifact declares its own partial freshness,
    # and both lists land in refresh_status.json for the \u00a78.1 modal.
    stale_pulls: list = []
    fresh_pulls: list = []
    ...
    try:
        ...
        # point. _list_pulls parses pulls.json, so a malformed document
        # surfaces under _PHASE_PULL_LOAD before any pull runs -- and
        # unlike the Python module it replaced, that phase cannot execute
        # anything at all. Once we have the pull names, each iteration
        # retags failing_phase to the specific pull about to run.
        pull_names = _list_pulls(folder, only=pulls_subset)
        ...
        for name in pull_names:
            ...
            try:
                ...
                # cycle-fatal one so the modal renders it unchanged.
                ...
                print(
                    f"    pull {name}  \u2717  {klass}: {exc}  "
                    f"(isolated; dataset keeps last-cycle bytes)",
                    flush=True,
                )
                print(
                    f"WARN pull {name} failed (isolated): "
                    f"{type(exc).__name__}: {exc}",
                    file=sys.stderr, flush=True,
                )
                print(traceback.format_exc(), file=sys.stderr, flush=True)
            else:
                fresh_pulls.append(name)
                print(
        ...
        # data and the pill would claim a refresh that moved nothing.
        if pull_names and not fresh_pulls:
            # Attribute the aggregate to the phase, not to whichever pull
            # happened to be last -- the per-pull entries above already
            # name each individual failure.
            failing_phase = _PHASE_PULL_PREFIX
            raise RuntimeError(
                f"refresh_runner: all {len(pull_names)} pulls failed "
                f"({', '.join(stale_pulls)}); skipping build so the "
                f"freshness stamp is not advanced over unchanged data"
            )
        if stale_pulls:
            print(
                f"    {len(fresh_pulls)}/{len(pull_names)} pulls fresh; "
                f"stale: {', '.join(stale_pulls)}",
                flush=True,
            )
        ...
        # time>" off this value. stale_pulls rides along so the artifact
        # declares which datasets are carrying last-cycle data.
        ...
        if mode_norm == "light":
            manifest = build_dashboard_data_only(
                ...
                stale_pulls=stale_pulls or None,
            )
        else:
        ...
        # anchor passed into build_dashboard byte-for-byte. A partial
        # cycle did advance the served bytes, so last_refreshed moves;
        # last_refresh_status carries the qualifier.
        failing_phase = _PHASE_REGISTRY
        cycle_status = "partial" if stale_pulls else "success"
        print(f"[PHASE 3/3] registry stamp", flush=True)
        _update_registry(kerberos, dashboard_id, cycle_status,
                          refreshed_at=cycle_iso)
        print(
            f"    last_refreshed={cycle_iso}  status={cycle_status}  \u2713",
            flush=True,
        )
    ...
    if stale_pulls:
        final_payload["stale_pulls"] = list(stale_pulls)
        final_payload["fresh_pulls"] = list(fresh_pulls)
    if log_path:
    ...
    stale_note = (
        f"  stale_pulls={len(stale_pulls)}/"
        f"{len(stale_pulls) + len(fresh_pulls)}"
        if stale_pulls else ""
    )
    _banner(
        f"runner done -- status={final_status}  "
        f"elapsed={elapsed:.2f}s  errors={len(errors)}{stale_note}"
    )
    if final_status == "success":
        return 0
    if final_status == "review_required":
        return 2
    # partial exits non-zero: the cycle is not healthy, and Django's
    # spawn handler treats a non-zero exit as "surface this to the user".
    # The browser branches on refresh_status.json's status field, not on
    # the exit code, so the partial modal still renders its own path.
    return 1
...
def main(argv: Optional[list] = None) -> int:
    ...
    parser.add_argument(
        "--pulls", nargs="+", default=None, metavar="NAME",
        help="Refresh only these declared pulls (document order). "
              "An undeclared name is an error, not a no-op.",
    )
    args = parser.parse_args(argv)
    return run(
        args.folder, log_path=args.log_path, mode=args.mode,
        pulls_subset=args.pulls,
    )
