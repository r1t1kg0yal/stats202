...
Public surface PRISM imports:
    ...
    from dashboards import (
        run_pull,                  # run ONE declared pull (in-process)
        ...
        refresh_dashboard,         # every pull + build_dashboard (full)
        light_refresh,             # every pull + build_dashboard_data_only
...
Python runtime dependencies are stdlib + pandas + numpy. ``scripts/
build.py`` is the only persisted Python a dashboard has, and it derives
rather than retrieves; its exec namespace binds ``s3_manager``, ``pd``
and ``np`` and nothing else. What a dashboard fetches is declared in
``scripts/pulls.json`` and executed by the engine, so the CSV stems a
refresh will write are readable without running anything.
