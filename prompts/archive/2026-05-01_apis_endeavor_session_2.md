...
reply_folded_into:
  ...
  - prism/api-clients.md (§4 full rewrite — init.py membership, sandbox injection, pull_nyfed_data exception, 4-clients-missing-from-init, 7-clients-not-injected; §6 unmapped clients confirmed; §7 bundles confirmed; §10 _USE_GS_PROXY tri-modal; §11 sentinels cleared, F19/F20/F21 opened)
  - prism/code-sandbox.md (§2.5 + §2.14 cleanup)
  - prism/codebase-tree.md (§3.4 expanded with mcp/ top-level files)
  - prism/_changelog.md (2026-05-01 (latest) entry — F8/F9/F10/F11/F12/F14/F15/F16/F17/F18 RESOLVED; F7/F13 PARTIAL; F19/F20/F21 NEW; C38-C42 added)
status: USED — every WARN sentinel from the Session 1 entry was either
  RESOLVED or remapped to a structurally-equivalent open item (F19/
  F20/F21) that is now blocked on either a future targeted re-check
  or on PRISM's verbatim-paste policy rather than on this round-trip.
key_findings:
  - tri-modal _USE_GS_PROXY (hardcoded / import-availability / no flag)
  - six clients are direct (no GS proxy at all): cftc, congress,
    federal_register, usitc, ofac, openfigi
  - sandbox injection asymmetry: __init__.py != script_exec_tools.py
    membership; pull_nyfed_data is function-injected (not module);
    7 clients not injected
  - PRISM declined verbatim source pastes — structural facts only
---

Title: API client architecture + GS proxy transport layer — verbatim introspection

I'm building a plug-and-play model for `projects/apis/` analogous to
`projects/altair/altair-payload/` and `projects/echarts/echarts-payload/`,
